...
exports.parseSuiteSource = function (src) {
    ...
    Object.keys(context).forEach( function (key) {
        if (context[key] && context[key].exportConfig) {
            config = context[key].exportConfig();
        }
    });
 ...
 *
 * Currently this involves doing one check:
 *   1) A test file cannot include any require() statements.
 *
...
exports.validateSuite = function (src) {
    var parsed = esprima.parse(src, { tokens: true }),
        hasRequire;

    hasRequire = parsed.tokens.some(function (token, index, arr) {
        var next;
