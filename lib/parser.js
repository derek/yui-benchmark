...
function prettifyResults (results) {
        ...
        refs.forEach(function (ref) {
            ...
            for (agent in data[test]) {
                ...
                if (data[test][agent].refs[ref].slower) {
                    row.push(data[test][agent].refs[ref].average.toFixed(2) + ' (' + data[test][agent].refs[ref].slower.toFixed(0) + '%)');
                }
                else {
                    row.push(data[test][agent].refs[ref].average.toFixed(2) + ' (' + log.color('Fastest', 'green') + ')');
                }
            }
    ...
    for(result in results) {
        refs[results[result].task.sha] = results[result].task.ref;
    }
