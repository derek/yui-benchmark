        ...
        refs.forEach(function (ref) {
            ...
            for (agent in data[test]) {
                ...
                if (data[test][agent].refs[ref].slower) {
                    row.push(data[test][agent].refs[ref].average.toFixed(2) + 'hz ±' + data[test][agent].refs[ref].errorMargin.toFixed(2) + '% (' + data[test][agent].refs[ref].slower.toFixed(0) + '% slower)');
                }
                else {
                    row.push(data[test][agent].refs[ref].average.toFixed(2) + 'hz ±' + data[test][agent].refs[ref].errorMargin.toFixed(2) + '% (' + log.color('Fastest', 'green') + ')');
                }
            }
