...
exports.yui = function (req, res) {
    var task = Task.findTaskById(req.params.taskID),
        srcPath = path.join((task ? task.dir : options.yuipath), req.params[0]);
...
exports.resultsData = function (req, res) {
    ...
    // tasks = require('/Users/derek/benchmark_data/data');

    out.meta.component = tasks[0].rawData.component;
    out.meta.name = tasks[0].rawData.name;
    ...
    // tasks.forEach(function (task) {
    //     data = {};
    //     data.category = task.ref.substring(0, 6);
    //     data[task.result.UA] = task.result.value;
