...
var EventEmitter = require('eventemitter2').EventEmitter2,
    ...
    parseSuiteSource = utilities.parseSuiteSource,
    // validateSuite = utilities.validateSuite,
...
proto.init = function (config) {
    ...
    this.runner = (this.config.node ? 'node' : 'web');
 ...
 */
proto.bindListeners = function bindListeners (callback) {
...
proto.findYUISeed = function findYUISeed (callback) {
    var self = this,
        config = self.config,
        cwd = (config['yui-repo'] || process.cwd()),
        suite = self.parsedSuite;

    if (!suite.yui) {
        callback(null);
    }
    else {
        exec('git', ['rev-parse', '--show-toplevel'], cwd, function handleExecComplete(err, stdout) {
...
/**
 * Gathers details about the target refs (date, ref, sha, rebuild)
 * and populates refTable
 *
...
proto.parseSuite = function createTasks (callback) {
    var self = this,
        config = self.config,
        suiteSrc = fs.readFileSync(config.source);

    // Skipping this for now
    // if (!validateSuite(suiteSrc)) {
        // throw new Error('Source file failed validation. Script cannot contain a `require()`.');
    // }

    self.parsedSuite = parseSuiteSource(suiteSrc);
...
proto.createTasks = function createTasks (callback) {
    ...
    refsByDate.forEach(function (refData) {
        for (i=0; i < iterations; i++) {
            ...
            if (repo) {
                if (ref === WIP) {
                    buildPath = path.join(repo, 'build');
                }
                else {
                    buildPath = path.join(repo, '.builds', ref + '-' + sha);
                }
            }
            ...
            if (suite.code) {
                if (fs.existsSync(path.join(sourceDir, suite.code))) {
                    suite.code = fs.readFileSync(path.join(sourceDir, suite.html), 'utf-8');
                }
            }

            suite.runner = self.runner;
...
proto.prepRepo = function prepRepo (ref, completeTask) {
    ...
    seedExists(function (exists) {
        if (exists) {
            ...
            log.info('%s: Creating seed. This could take a few minutes.', ref);
...
proto.handleAgentResult = function handleAgentResult (agentName, response) {
    ...
    log.info('Got result from %s', agentName);
    ...
    results.forEach(function handleEachResult (result) {
        ...
        self.emit('result', {
            unit: "Hz",
            name: htmlEntitiesDecode(result.name),
            value: result.value,
            stats: result.stats,
            UA: agentName,
            ref: task.ref,
...
proto.boot = function () {
    var self = this,
        config = self.config,
        fnSeries = [
            self.bindListeners.bind(self),
            self.parseSuite.bind(self),
    ...
    async.series(fnSeries, bootComplete);
...
proto.executeTests = function executeTests () {
    ...
    if (config.node) {
        tasks.forEach(function (task) {
            ...
            log.debug('Suite "' + task.name + '" ...');

            vm.runInNewContext(task.code, context);

            context.suite.on("complete", self.handleBatchComplete.bind(self));

            context.suite.on("cycle", function (e) {
                var results = e.target;

                // Don't need this.  Too much data.
                delete results.stats.sample;
