...
proto.boot = function () {
    ...
    async.series(bootSeries, function (err) {
        if (err) {
            self.emit('error', err);
        }
        else {
...
proto.findYUI = function findYUI(callback) {
    ...
    exec('git', ['rev-parse', '--show-toplevel'], cwd, function(err, stdout) {
    ...
    function prepSha (ref, next) {
        getCommitDate(ref, function (err, date) {
            ...
            if (ref === WIP) {
                ...
                getCommitSHA(ref, function (error, sha) {
            ...
            function getCommitSHA(ref, callback) {
                exec('git', ['rev-parse', ref], yuipath, function(err, stdout) {
                    var sha = stdout.trim();
        ...
        function getCommitDate(ref, callback) {
            var args = ['show', '--format="%ci"'];

            if (ref !== WIP) {
                args.push(ref);
            }

            exec('git', args, yuipath, function(err, stdout) {
...
proto.createTasks = function createTasks (callback) {
    ...
    vm.runInNewContext(fs.readFileSync(path.join(__dirname, '../templates/perf-suite.js')) + fs.readFileSync(config.source), context);
    suite = context.suite.exportConfig();

    log.info('Compiling "%s" (containing %d tests) for %d refs (%s)', suite.name, suite.tests.length, refs.length, refs.join(', '));
...
proto.gatherTestURLs = function gatherTestURLs (callback) {
    ...
    self.testURLs = tasks.map(function (task) {
        return task.url;
    });
...
proto.prepRepo = function prepRepo (ref, completeTask) {
    ...
    async.series(prepSequence, function(){
        seedExists(function(exists) {
            ...
            if (exists) {
                ...
                msg = '%s: Unable to find seed @ ' + seedPath;
            }
...
proto.startYeti = function startYeti (callback) {
    ...
    log.info('Listening at http://127.0.0.1:%d', port);
...
proto.handleResult = function handleResult (agentName, response) {
    ...
    log.info('Got result for task %d from %s', taskID, agentName);
