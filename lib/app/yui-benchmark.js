...
/**
 * Gather information about refs (date, ref, sha, rebuild)
 *
 * @private
 * @param {Function} Callback to execute upon completion
 */
proto.prepRefs = function prepRefs (callback) {
    var self = this,
        config = self.config,
...
proto.createTasks = function createTasks (callback) {
    ...
    log.info('Compiling "%s" (contains %d tests) for %d refs (%s)', suite.name, suite.tests.length, refs.length, refsByDateStr);
