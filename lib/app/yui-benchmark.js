...
proto.prepRepo = function prepRepo (ref, completeTask) {
    ...
    log.debug('tmproot: %s', tmproot);
    log.debug('ref: %s', ref);
    log.debug('seedPath: %s', seedPath);
    log.debug('buildDir: %s', buildDir);
    log.debug('repositoryPath: %s', repositoryPath);
    ...
    async.series(prepSequence, function(err, results){
        ...
        seedExists(function(exists) {
            if (exists) {
                log.info(ref + ': Seed found!');
                completeTask(null, ref + ': Seed found!');
            }
            else {
