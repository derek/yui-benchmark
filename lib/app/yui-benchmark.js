...
proto.prepRepo = function prepRepo (ref, callback) {
    ...
    if (fs.existsSync(seedPath)) {
        ...
        async.series([
            ...
            function(next){ myExec('git fetch origin', repositoryPath, next); },
            function(next){ myExec('git checkout ' + sha, repositoryPath, next); },
            function(next){ myExec('yogi build', repositoryPath, next); },
            function(next){ myExec('cp -r ' + path.join(repositoryPath, 'build') + ' ./', buildDir, next); }
...
proto.compileTests = function compileTests (callback) {
    var self = this,
        config = self.config,
        compile = require('jsperf-tools/lib/compile'),
    ...
    suites.forEach(function (suite) {
        suite.yui.src = '/yui/build/yui/yui.js';
        suite.html = '\n<script src="/yeti/public/inject.js"></script>';
        suite.javascript = '\nvar genericYetiDriver = true;\nvar results=[];\nsuite.on("complete", function (e) {\n\twindow.yetiComplete({results: results});\n});\nsuite.on("cycle", function (event) {\n\tvar bench=event.target;\nresults.push({name:bench.name,unit:"hz",value:bench.hz,stats:bench.stats});\n});';
    });

    this.tests = compile(suites);
...
proto.handleResult = function handleResult (agentName, response) {
    ...
    results.forEach(function handleEachResult (result) {
        var t = findTaskById(tasks, taskID);
