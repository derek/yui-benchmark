...
proto.findYUI = function findYUI(callback) {
    ...
    myExec('git rev-parse --show-toplevel', cwd, function(error, stdout) {
...
proto.prepSHAs = function prepSHAs (callback) {
    ...
    async.each(config.refs, function (ref, next) {
        myExec('git rev-parse ' + ref, self.yuipath, function(err, stdout) {
            sha = stdout.trim();
...
proto.prepRepo = function prepRepo (ref, callback) {
    ...
    if (fs.existsSync(seedPath)) {
        ...
        async.series([
            ...
            function(next){ myExec('yogi build', repositoryPath, next); }, // Using `yogi build` instead of shifter because yogi can build via ant and shifter.
