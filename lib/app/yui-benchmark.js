...
proto.prepSHAs = function prepSHAs (callback) {
    ...
    async.each(config.refs, function (ref, next) {
        exec('git rev-parse ' + ref, {cwd: self.yuipath}, function(err, stdout) {
            sha = stdout.trim();
...
proto.createTasks = function createTasks (callback) {
    ...
    refs.forEach(function (ref) {
        suites.forEach(function (suite) {
            ...
            log.info('Compiling "%s" (%d tests)', suite.title, suite.tests.length);
            ...
            tasks.push({
                ...
                buildPath: (ref === "WIP" ? path.join(yuipath, 'build') : path.join(yuipath, '.builds', ref + '-' + sha, 'build'))
            });
        });
    });
...
function simpleExec(args, cwd, callback) {
    ...
    log.debug('simpleExec: %s %s %s', cmd, args, cwd);
