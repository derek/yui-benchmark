...
function prettify (results) {
        ...
        refs.forEach(function (ref) {
            ...
            for (agent in data[test]) {
                ...
                if (data[test][agent].refs[ref].slower) {
                    ...
                    row.push(data[test][agent].refs[ref].average.toFixed(2) + data[test][agent].refs[ref].unit + ' ±' + data[test][agent].refs[ref].errorMargin.toFixed(2) + '% (Fastest)');
                }
            }
    ...
    for (sha in refs) {
        table.push([refs[sha], (sha === WIP ? '(local development tree)' : sha)]).toString();
    }
