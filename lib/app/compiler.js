...
module.exports = function (suite, sourceDir, templateSrc) {
...
function templateToHTML (values, templateSrc) {
    var templatePath = path.join(__dirname, '../templates/', templateSrc),
        template;
...
function normalize (suite) {
    ...
    */
    ...
    suite.title = String(suite.title);
    suite.slug = suite.slug || suite.title;
