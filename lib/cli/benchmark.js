...
var path = require("path"),
    ...
    getProcessList = utilities.getProcessList,
...
// Before we begin, we need to first ensure that there are no other phantomjs processes running
getProcessList(function(err, processes){
    processes.forEach(function(process) {
        if (process.cmd.match(/yui-benchmark\/scripts\/load_url.js/)) {
            log.error('A phantomjs load_url.js process is already running.  Kill it with fire!  Found: ' + process.cmd);
            exit(false);
        }
    });
...
function spawnPhantom() {
    ...
    // phantomProcess.stdout.on('data', (log.debug).bind(log));
}
